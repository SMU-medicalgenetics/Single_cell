...
#added on 20190806,HP1P 5563 cells
ident=read.csv('cellrangerRkit/Graph-based.with_clusterID.csv')
ident$Barcode=gsub('-1','',ident$Barcode)
rownames(ident)=ident$Barcode
HP1P=RunPCA(HP1P,pc.genes=HP1P@var.genes,do.print=T,pcs.print=0,genes.print=0)
HP1P=RunTSNE(HP1P, do.fast = TRUE,dims.use = 1:10)
HP1P@meta.data$group=ident[rownames(HP1P@meta.data),'Graph.based']
HP1P=SetAllIdent(HP1P,id = 'group')
markers_cluster=FindAllMarkers(HP1P,only.pos=T,test.use="roc")
write.table(markers_cluster,"HP1P_seurat.markers_cluster.roc.txt",sep="\t",row.names=F)
