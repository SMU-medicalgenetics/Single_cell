...
write.table(selected_markers,paste0("HP2/HP2.","topMarkersForEachStage.Blood.selected_markers.txt"),sep="\t",row.names = F)
...
seurat <- AddModuleScore(seurat, genes.list = modulegenes, enrich.name = "HP2.proerythroblast")
modulegenes<- sigGenes.basophilic
modulegenes<- list(intersect(rownames(seurat@raw.data), modulegenes))
seurat <- AddModuleScore(seurat, genes.list = modulegenes, enrich.name = "HP2.basophilic")
modulegenes<- sigGenes.polychromatic
modulegenes<- list(intersect(rownames(seurat@raw.data), modulegenes))
seurat <- AddModuleScore(seurat, genes.list = modulegenes, enrich.name = "HP2.polychromatic")
modulegenes<- sigGenes.orthochromatic
modulegenes<- list(intersect(rownames(seurat@raw.data), modulegenes))
seurat <- AddModuleScore(seurat, genes.list = modulegenes, enrich.name = "HP2.orthochromatic")
FeaturePlot(seurat, cols.use = c("yellow", "red"), reduction.use = "tsne"
            , features.plot =c("HP2.proerythroblast1","HP2.basophilic1","HP2.polychromatic1",'HP2.orthochromatic1')
...
plb<-uc_id
plb[uc_id %in% c(4,5)]<-'basoE'
plb[uc_id %in% c(2,3,10)]<-'polyE'
plb[uc_id %in% c(0,1,6,7,8,9)]<-'orthoE'
...
proE_cells=FeaturePlot(seurat,cols.use=c("yellow","red"),reduction.use="tsne",features.plot="HP2.proerythroblast1",do.identify = T,dark.theme =T)
seurat@meta.data[rownames(seurat@meta.data) %in% proE_cells,"putative_stage"]<-"proE" #28 cells

table(seurat@meta.data$putative_stage)
sum(table(seurat@meta.data$putative_stage))
dim(seurat@meta.data)
HP2_cell_summary=table(seurat@meta.data$putative_stage)
...
save(seurat,file="HP2/h2_seurat.rdata")
